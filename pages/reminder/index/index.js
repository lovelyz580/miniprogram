	...
	async getReminders(refresh = false) {
		...
		try {
			const res = await request({
				url: urls.reminderList,
				method: 'POST',
				data: {
					reminderType: currentCategory === 'all' ? '' : currentCategory,
					page,
					pageSize
				}
			})
			...
			if (res.code === 200) {
				const allReminders = (res.rows || []).map(item => ({
					...item,
					categoryText: this.getCategoryText(item.reminderType),
		...
		try {
			if (item.isCompleted) {
				...
				await request({
					...
					data: {
						reminderId: id,
						completedDate: formatTime(new Date(), 'yyyy-MM-dd')
					}
				})
			}
